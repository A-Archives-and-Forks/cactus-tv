...
> v1.3.0 Playback Stability：普通拖拽只提交一次，不再反复停止和重启 HLS；只有目标位置真正显示的新视频帧才会写入稳定进度。播放器会识别 MPEG-TS 时间戳回绕和异常超长时长，续播首分片失败时按真实分片边界有限重试，并限制同一播放会话内的恢复次数。实验性 Clean Stream 默认关闭。
